...
const epiRanking = [
  { name: 'Luva de proteção', amount: 126, color: '#1677D2' },
  { name: 'Óculos de segurança', amount: 98, color: '#1699C2' },
  { name: 'Capacete', amount: 74, color: '#E5B940' },
  { name: 'Protetor auricular', amount: 56, color: '#65D5CC' },
];
...
export function DashboardScreen({ onNavigate }: { onNavigate: (screen: ScreenName) => void }) {
  ...
  return <ScrollView contentContainerStyle={[styles.content, isCompact && styles.compactContent]}>
    ...
    <View style={styles.metricsGrid}>
      <MetricCard label="Trabalhadores ativos" value="128" detail="4 novos este mês" accent="#1677D2" icon="◉" onPress={() => onNavigate('Trabalhadores')} />
      <MetricCard label="EPIs sincronizados" value="24" detail="22 com CA válido • base oficial" accent="#1699C2" icon="▣" />
  ...
  <html lang="pt-BR">
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        body { color: #162B45; font-family: Arial, Helvetica, sans-serif; font-size: 11px; margin: 0; }
        ...
        .header { align-items: flex-start; border-bottom: 1px solid #D5E4F3; display: flex; justify-content: space-between; padding-bottom: 22px; }
        .brand { align-items: center; display: flex; gap: 10px; }
        .brand-mark { align-items: center; background: #1677D2; border-radius: 7px; color: #e5b940; display: flex; font-size: 21px; font-weight: bold; height: 35px; justify-content: center; width: 35px; }
        .brand-name { color: #1677D2; font-size: 16px; font-weight: bold; }
        .report-type { color: #7489A0; font-size: 9px; letter-spacing: 1.1px; margin-top: 2px; }
        h1 { color: #12355B; font-size: 25px; letter-spacing: -.5px; margin: 24px 0 5px; }
        .subtitle { color: #536B83; font-size: 12px; margin: 0; }
        .period { background: #F6FAFF; border-left: 3px solid #1699C2; color: #12355B; font-size: 10px; font-weight: bold; margin-top: 8px; padding: 7px 10px; }
        .period small { color: #536B83; display: block; font-size: 8px; font-weight: bold; letter-spacing: .7px; margin-bottom: 2px; }
        .section-title { color: #1677D2; font-size: 14px; font-weight: bold; margin: 24px 0 4px; }
        .section-caption { color: #7489A0; margin: 0 0 12px; }
        .kpis { display: grid; gap: 8px; grid-template-columns: repeat(3, 1fr); }
        .kpi { border: 1px solid #D5E4F3; border-radius: 8px; min-height: 72px; padding: 10px; }
        .kpi-label { color: #536B83; font-size: 9px; }
        .kpi-value { color: #12355B; font-size: 21px; font-weight: bold; margin-top: 9px; }
        .kpi-alert .kpi-value { color: #b23830; }
        .conformity { background: #F6FAFF; border: 1px solid #D5E4F3; border-radius: 9px; display: flex; gap: 20px; margin-top: 18px; padding: 14px; }
        .conformity-value { color: #1677D2; font-size: 28px; font-weight: bold; min-width: 87px; }
        .conformity-value span { color: #536B83; display: block; font-size: 9px; font-weight: normal; margin-top: 2px; }
        .conformity-content { flex: 1; }
        .conformity-content strong { color: #12355B; display: block; margin-bottom: 7px; }
        .progress { background: #D5E4F3; border-radius: 5px; height: 8px; overflow: hidden; }
        .progress div { background: #1699C2; height: 100%; width: 94.5%; }
        .progress-note { color: #536B83; font-size: 9px; margin-top: 6px; }
        ...
        th { background: #1677D2; color: #fff; font-size: 9px; letter-spacing: .3px; padding: 9px; text-align: left; }
        th:first-child { border-radius: 6px 0 0 0; } th:last-child { border-radius: 0 6px 0 0; }
        td { border-bottom: 1px solid #D5E4F3; color: #365879; padding: 9px; }
        tr:nth-child(even) td { background: #F8FBFF; }
        .rank { color: #1677D2; display: inline-block; font-weight: bold; width: 20px; }
        .number { color: #12355B; font-weight: bold; text-align: right; width: 42px; }
        .bar-track { background: #EEF5FC; border-radius: 4px; height: 7px; overflow: hidden; width: 100%; }
        .bar { background: #1699C2; border-radius: 4px; height: 100%; }
        .stock { border: 1px solid #D5E4F3; border-radius: 8px; padding: 15px; }
        .stock-total { color: #1677D2; font-size: 26px; font-weight: bold; }
        .stock-total span { color: #536B83; font-size: 9px; font-weight: normal; }
        .stock-row { border-top: 1px solid #D5E4F3; display: flex; justify-content: space-between; margin-top: 11px; padding-top: 11px; }
        .stock-row strong { font-size: 13px; } .healthy { color: #1677D2; } .attention { color: #a16b13; } .critical { color: #b23830; }
        ...
        .alert strong { color: #12355B; display: block; font-size: 10px; } .alert span { color: #536B83; display: block; font-size: 9px; margin-top: 2px; }
        ...
        .alert-info { background: #f0f7f7; } .alert-info .alert-number { background: #E1F4F3; color: #087a73; }
        .footer { bottom: 20px; color: #7489A0; font-size: 8px; left: 42px; position: absolute; right: 42px; }
        .footer-line { background: #D5E4F3; height: 1px; margin-bottom: 8px; }.footer-content { display: flex; justify-content: space-between; }
    ...
    <body>
      <div class="page">
        ...
        <div class="header"><div><div class="brand"><div class="brand-mark">H</div><div><div class="brand-name">Handsafe</div><div class="report-type">GESTÃO DE SEGURANÇA</div></div></div><h1>Relatório executivo</h1><p class="subtitle">Indicadores consolidados de estoque, entregas e conformidade NR-06.</p></div><div class="period"><small>PERÍODO ANALISADO</small>${period}</div></div>
        ...
        <div class="footer"><div class="footer-line"></div><div class="footer-content"><span>Handsafe - uso interno</span><span>Gerado em ${generatedAt}</span></div></div>
      </div>
    </body>
  </html>`;
}
...
const styles = StyleSheet.create({
  ...
  hero: { alignItems: 'flex-end', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }, heroNarrow: { alignItems: 'flex-start', flexDirection: 'column', gap: 14 }, heroText: { flexShrink: 1 }, eyebrow: { color: '#1699C2', fontSize: 11, fontWeight: '800', letterSpacing: 1.2, marginBottom: 5 }, title: { color: '#12355B', fontSize: 28, fontWeight: '800', letterSpacing: -0.6 }, subtitle: { color: '#5C7187', fontSize: 14, lineHeight: 20, marginTop: 5 }, panelActions: { alignItems: 'flex-end', flexDirection: 'row', flexShrink: 1, gap: 9 }, panelActionsNarrow: { alignItems: 'stretch', flexDirection: 'column', width: '100%' }, periodFilter: { flexBasis: 186, flexGrow: 0, width: 186 }, pdfButton: { alignItems: 'center', backgroundColor: '#12355B', borderRadius: 8, flexDirection: 'row', gap: 7, height: 43, justifyContent: 'center', paddingHorizontal: 14 }, pdfButtonDisabled: { backgroundColor: '#91A8C1' }, pdfButtonIcon: { color: '#65D5CC', fontSize: 18, fontWeight: '800' }, pdfButtonText: { color: '#FFFFFF', fontSize: 13, fontWeight: '800' },
  metricsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginBottom: 14 }, metricCard: { backgroundColor: '#FFFFFF', borderColor: '#D5E4F3', borderRadius: 13, borderWidth: 1, flexBasis: 185, flexGrow: 1, minHeight: 152, overflow: 'hidden', padding: 16 }, pressableCard: { opacity: 0.78 }, metricIcon: { alignItems: 'center', borderRadius: 8, height: 28, justifyContent: 'center', position: 'absolute', right: 14, top: 14, width: 28 }, metricIconText: { color: '#FFFFFF', fontSize: 16, fontWeight: '800' }, metricLabel: { color: '#536B83', fontSize: 12, fontWeight: '600', maxWidth: '75%' }, metricValue: { color: '#12355B', fontSize: 27, fontWeight: '800', letterSpacing: -.8, marginTop: 17 }, metricDetail: { color: '#7489A0', fontSize: 11, marginTop: 8 }, metricLink: { color: '#1677D2', fontSize: 11, fontWeight: '800', marginTop: 8 },
  primaryRow: { flexDirection: 'row', gap: 14 }, primaryColumn: { flexDirection: 'column' }, card: { backgroundColor: '#FFFFFF', borderColor: '#D5E4F3', borderRadius: 13, borderWidth: 1, padding: 18 }, fullWidth: { alignSelf: 'stretch', width: '100%' }, consumptionCard: { flex: 1.35, minWidth: 0 }, complianceCard: { flex: 1, minWidth: 0 }, chartHeader: { marginBottom: 18 }, cardTitle: { color: '#12355B', fontSize: 16, fontWeight: '800' }, cardSubtitle: { color: '#7489A0', fontSize: 12, marginTop: 4 },
  chartArea: { flexDirection: 'row', height: 182 }, axis: { height: 154, justifyContent: 'space-between', paddingBottom: 1, width: 25 }, axisLabel: { color: '#91A8C1', fontSize: 9 }, bars: { borderBottomColor: '#DDE8F2', borderBottomWidth: 1, flex: 1, flexDirection: 'row', gap: 5, height: 155, justifyContent: 'space-between' }, barColumn: { alignItems: 'center', flex: 1, height: 180, justifyContent: 'flex-end', minWidth: 0 }, barTrack: { backgroundColor: '#EEF5FC', borderRadius: 4, height: 154, justifyContent: 'flex-end', overflow: 'hidden', width: '100%' }, bar: { backgroundColor: '#1677D2', borderRadius: 4, minHeight: 4, width: '100%' }, barLabel: { color: '#7489A0', fontSize: 9, marginTop: 7 }, chartFooter: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }, legend: { alignItems: 'center', flexDirection: 'row', gap: 6 }, legendDot: { backgroundColor: '#1677D2', borderRadius: 5, height: 8, width: 8 }, legendText: { color: '#60778F', fontSize: 11 }, chartTotal: { color: '#12355B', fontSize: 11, fontWeight: '700' },
  complianceBody: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-around', minHeight: 163 }, scoreRing: { alignItems: 'center', borderColor: '#1677D2', borderRadius: 75, borderWidth: 10, height: 132, justifyContent: 'center', width: 132 }, scoreValue: { color: '#12355B', fontSize: 24, fontWeight: '800' }, scoreLabel: { color: '#7489A0', fontSize: 11, marginTop: 2 }, complianceLegend: { gap: 13, marginLeft: 12 }, legendItem: { alignItems: 'center', flexDirection: 'row', gap: 7 }, legendItemLabel: { color: '#5C7187', fontSize: 11, minWidth: 94 }, legendItemValue: { color: '#183B61', fontSize: 12, fontWeight: '800' }, progress: { backgroundColor: '#DDE8F2', borderRadius: 5, flexDirection: 'row', height: 7, marginTop: 14, overflow: 'hidden' }, progressSafe: { backgroundColor: '#12355B', width: '94.5%' }, progressWarning: { backgroundColor: '#65D5CC', flex: 1 }, note: { color: '#7489A0', fontSize: 11, lineHeight: 16, marginTop: 9 },
  secondaryRow: { flexDirection: 'row', gap: 14, marginTop: 14 }, rankingCard: { flex: 1.15, minWidth: 0 }, stockCard: { flex: .85, minWidth: 0 }, rankRow: { alignItems: 'center', flexDirection: 'row', gap: 9, marginBottom: 13 }, rankName: { color: '#365879', fontSize: 11, width: 113 }, rankTrack: { backgroundColor: '#F6FAFF', borderRadius: 4, flex: 1, height: 9, overflow: 'hidden' }, rankBar: { borderRadius: 4, height: '100%' }, rankValue: { color: '#12355B', fontSize: 11, fontWeight: '800', textAlign: 'right', width: 25 }, cardCaption: { color: '#7489A0', fontSize: 10, marginTop: 3 }, stockBars: { flexDirection: 'row', gap: 10, justifyContent: 'space-between', marginBottom: 19 }, stockSegment: { alignItems: 'center', flex: 1 }, stockPercent: { fontSize: 19, fontWeight: '800' }, stockLabel: { color: '#536B83', fontSize: 11, fontWeight: '700', marginTop: 3 }, stockCount: { color: '#7489A0', fontSize: 10, marginTop: 2 }, stockSummary: { alignItems: 'baseline', backgroundColor: '#F6FAFF', borderRadius: 8, flexDirection: 'row', gap: 7, justifyContent: 'center', padding: 11 }, stockSummaryValue: { color: '#1677D2', fontSize: 23, fontWeight: '800' }, stockSummaryLabel: { color: '#536B83', fontSize: 11 },
  alertSection: { backgroundColor: '#FFFFFF', borderColor: '#D5E4F3', borderRadius: 13, borderWidth: 1, marginTop: 14, padding: 18 }, sectionHeading: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 }, sectionTitle: { color: '#12355B', fontSize: 16, fontWeight: '800' }, sectionSubtitle: { color: '#7489A0', fontSize: 12, marginTop: 3 }, alertCount: { backgroundColor: '#FFF0EE', borderRadius: 14, overflow: 'hidden', paddingHorizontal: 9, paddingVertical: 5 }, alertCountText: { color: '#B23830', fontSize: 11, fontWeight: '800' }, alertGrid: { flexDirection: 'row', gap: 10 }, alertColumn: { flexDirection: 'column' }, alertCard: { alignItems: 'center', borderRadius: 9, borderWidth: 1, flex: 1, flexDirection: 'row', minWidth: 0, padding: 12 }, alert_danger: { backgroundColor: '#FFF4F2', borderColor: '#F2D2CD' }, alert_warning: { backgroundColor: '#FFF9E9', borderColor: '#F3E0A4' }, alert_info: { backgroundColor: '#F0F7F7', borderColor: '#D1E1F2' }, alertNumber: { alignItems: 'center', borderRadius: 20, fontSize: 15, fontWeight: '800', overflow: 'hidden', paddingHorizontal: 9, paddingVertical: 6 }, alertNumber_danger: { backgroundColor: '#FCE0DC', color: '#B23830' }, alertNumber_warning: { backgroundColor: '#F9E8B7', color: '#8B620B' }, alertNumber_info: { backgroundColor: '#E1F4F3', color: '#087A73' }, alertCopy: { flex: 1, marginLeft: 9, minWidth: 0 }, alertTitle: { color: '#12355B', fontSize: 12, fontWeight: '800' }, alertDescription: { color: '#536B83', fontSize: 10, lineHeight: 14, marginTop: 3 }, alertArrow: { color: '#7489A0', fontSize: 25, lineHeight: 25, marginLeft: 4 },
});
